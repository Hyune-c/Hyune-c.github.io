...
- `kr`, `jp` SaaS zone 과 고객사 airgap 환경을 함께 지원해야 했습니다
...
- SaaS 멀티 리전과 airgap 환경을 모두 고려한 실행 구조를 설계했습니다
    - `kr`, `jp` 등 SaaS zone 분리와 동시에 N개의 airgap 환경을 최소한의 작업으로 지원할 수 있는 기반을 만들었습니다

## 설계 및 구현

같은 코드 베이스로 SaaS 멀티 리전(kr · jp) 과 N 개의 airgap 환경을 동시에 지원하기 위해, profile 기반 컴포넌트 조립과 Facade 중심 job lifecycle 제어를 적용했습니다.
